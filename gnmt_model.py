...
class GNMTModel(attention_model.AttentionModel):
    ...
    def __init__(self, hparams, mode, iterator,
        ...
        super(GNMTModel, self).__init__(hparams=hparams, mode=mode, iterator=iterator,
    ...
    def _build_encoder(self, hparams):
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            uni_cell = model_helper.create_rnn_cell(unit_type=hparams.unit_type,
    ...
    def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state, source_sequence_length):
        ...
        attention_mechanism = attention_model.create_attention_mechanism(attention_option, num_units, memory, source_sequence_length)
        ...
        alignment_history = (self.mode == tf.contrib.learn.ModeKeys.INFER and
                             beam_width == 0)
        attention_cell = tf.contrib.seq2seq.AttentionWrapper(attention_cell,
                                                             attention_mechanism,
        ...
        if hparams.pass_hidden_state:
            decoder_initial_state = tuple(zs.clone(cell_state=es) if isinstance(zs, tf.contrib.seq2seq.AttentionWrapperState) else es for zs, es in zip(cell.zero_state(batch_size, dtype), encoder_state))
        else:
