...
__all__ = ["create_infer_model", "load_data", "inference", "single_worker_inference", "multi_worker_inference"]


class InferModel(collections.namedtuple("InferModel", ("graph", "model", "src_placeholder", "iterator"))):
    pass
...
def _decode_inference_indices(model, sess, output_infer, output_infer_summary_prefix, inference_indices, tgt_eos, bpe_delimiter):
    ...
    with codecs.getwriter("utf-8")(tf.gfile.GFile(output_infer, mode="wb")) as trans_f:
        ...
        for decode_id in inference_indices:
            ...
            translation = nmt_utils.get_translation(nmt_outputs, sent_id=0, tgt_eos=tgt_eos, bpe_delimiter=bpe_delimiter)
...
def single_worker_inference(infer_model, ckpt, inference_input_file, inference_output_file, hparams):
    ...
    with tf.Session(graph=infer_model.graph, config=utils.get_config_proto()) as sess:
        ...
        sess.run(infer_model.iterator.initializer, feed_dict={infer_model.src_placeholder: infer_data})

        # Decode
        utils.print_out("# Start decoding")
        if hparams.inference_indices:
            _decode_inference_indices(loaded_infer_model, sess, output_infer=output_infer, output_infer_summary_prefix=output_infer, inference_indices=hparams.inference_indices, tgt_eos=hparams.eos, bpe_delimiter=hparams.bpe_delimiter)
        else:
            nmt_utils.decode_and_evaluate("infer", loaded_infer_model, sess, output_infer, ref_file=None, metrics=hparams.metrics, bpe_delimiter=hparams.bpe_delimiter, beam_width=hparams.beam_width, tgt_eos=hparams.eos)
...
def multi_worker_inference(infer_model, ckpt, inference_input_file, inference_output_file, hparams, num_workers, jobid):
    ...
    with tf.Session(graph=infer_model.graph, config=utils.get_config_proto()) as sess:
        ...
        loaded_infer_model = model_helper.load_model(infer_model.model, ckpt, sess, "infer")
        sess.run(infer_model.iterator.initializer, {infer_model.src_placeholder: infer_data})
