...
class EvalModel(collections.namedtuple("EvalModel", ("graph", "model", "src_file_placeholder", "tgt_file_placeholder", "iterator"))):
    pass
...
def run_internal_eval(eval_model, eval_sess, model_dir, hparams, summary_writer):
    ...
    with eval_model.graph.as_default():
        loaded_eval_model, global_step = model_helper.create_or_load_model(eval_model.model, model_dir, eval_sess, "eval")
    ...
    if hparams.test_prefix:
        ...
        test_ppl = _internal_eval(loaded_eval_model, global_step, eval_sess, eval_model.iterator, test_eval_iterator_feed_dict, summary_writer, "test")
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams, summary_writer, save_best_dev=True):
    ...
    dev_scores = _external_eval(loaded_infer_model, global_step, infer_sess, hparams, infer_model.iterator, dev_infer_iterator_feed_dict, dev_tgt_file, "dev", summary_writer, save_on_best=save_best_dev)
    ...
    if hparams.test_prefix:
        ...
        test_infer_iterator_feed_dict = {infer_model.src_placeholder: inference.load_data(test_src_file)}

        test_scores = _external_eval(loaded_infer_model, global_step, infer_sess, hparams, infer_model.iterator, test_infer_iterator_feed_dict, test_tgt_file, "test", summary_writer, save_on_best=False)
...
def train(hparams, scope=None, target_session="", single_cell_fn=None):
    ...
    train_model = create_train_model(model_creator, hparams, scope, single_cell_fn)

    if hparams.eval_on_fly:
        eval_model = create_eval_model(model_creator, hparams, scope, single_cell_fn)
        infer_model = inference.create_infer_model(model_creator, hparams,scope, single_cell_fn)
    ...
    utils.print_out("# Start step %d, lr %g, %s" % (global_step, loaded_train_model.learning_rate.eval(session=train_sess), time.ctime()), log_f)
    ...
    while global_step < num_train_steps:
        ...
        if global_step - last_stats_step >= steps_per_stats:
            ...
            utils.print_out("  global step %d lr %g step-time %.2fs wps %.2fK ppl %.2f %s" % (global_step, loaded_train_model.learning_rate.eval(session=train_sess), avg_step_time, speed, train_ppl, _get_best_results(hparams)), log_f)
        ...
        ##
        if (not hparams.eval_on_fly) and (global_step- last_snapshot_step >= steps_per_snapshot):
            last_snapshot_step = global_step
            utils.print_out("# Cihan: Saving Snapshot, global step %d" % global_step)
        ...
        if hparams.eval_on_fly and (global_step - last_external_eval_step >= steps_per_external_eval):
            last_external_eval_step = global_step
    ...
    if hparams.eval_on_fly:
        ...
        utils.print_out("# Final, step %d lr %g step-time %.2f wps %.2fK ppl %.2f, %s, %s" % (global_step, loaded_train_model.learning_rate.eval(session=train_sess), avg_step_time, speed, train_ppl, result_summary, time.ctime()), log_f)
        utils.print_time("# Done training!", start_train_time)

        utils.print_out("# Start evaluating saved best models.")
        for metric in hparams.metrics:
            best_model_dir = getattr(hparams, "best_" + metric + "_dir")
            result_summary, best_global_step, _, _, _, _ = run_full_eval(best_model_dir, infer_model, infer_sess, eval_model, eval_sess, hparams, summary_writer, sample_src_data, sample_tgt_data)
            utils.print_out("# Best %s, step %d step-time %.2f wps %.2fK, %s, %s" % (metric, best_global_step, avg_step_time, speed, result_summary, time.ctime()), log_f)
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data, tgt_data, iterator_src_placeholder, summary_writer):
    """Pick a sentence and decode."""
    decode_id = random.randint(0, len(src_data) - 1)
    utils.print_out("  # %d" % decode_id)

    iterator_feed_dict = {iterator_src_placeholder: [src_data[decode_id]]}
    ...
    translation = nmt_utils.get_translation(nmt_outputs, sent_id=0, tgt_eos=hparams.eos, bpe_delimiter=hparams.bpe_delimiter)
...
def _external_eval(model, global_step, sess, hparams, iterator, iterator_feed_dict, tgt_file, label, summary_writer, save_on_best):
    ...
    scores = nmt_utils.decode_and_evaluate(label, model, sess, output, ref_file=tgt_file, metrics=hparams.metrics, bpe_delimiter=hparams.bpe_delimiter, beam_width=hparams.beam_width, tgt_eos=hparams.eos, decode=decode)
    ...
    if decode:
        for metric in hparams.metrics:
            ...
            if save_on_best and scores[metric] > getattr(hparams, "best_" + metric):
                ...
                model.saver.save( sess, os.path.join( getattr(hparams, "best_" + metric + "_dir"), "translate.ckpt"), global_step=model.global_step)
